...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // ice is--> ice%20is
		...
		try {
			CloseableHttpResponse response = httpClient.execute(new HttpGet(url));//第一要知道url，第二要知道是get还是post
			if (response.getStatusLine().getStatusCode() != 200) {//没有解析的必要了，肯定是错误的
				return new ArrayList<>();
			}
			HttpEntity entity = response.getEntity();//得到body
			if (entity == null) {
				return new ArrayList<>();
			}
			BufferedReader reader = new BufferedReader(new InputStreamReader(entity.getContent()));//一段一段读
			StringBuilder responseBody = new StringBuilder();
			String line = null;
			while ((line = reader.readLine()) != null) {//readline是一行一行的读
